# Reid Alderson and Iva Pritišanac, November 2021
...
def read_AFDB(proteome):
    """ read pdb.gz files in an AlphaFold DB Proteome directory, return per-residue pLDDT scores """
    ...
    for r, d, f in os.walk(os.getcwd()):
        for file in f:
            if file.endswith(".pdb.gz"):
                ...
                fragment_num = int(file.split('-')[2][1:])      # get the fragment number (F1 -> 1, F2 -> 2, ...)
                pdb_file = gzip.open(file, "rt")                # read the pdb.gz file
                ...
                pLDDT_list.append([uniprot_ID, list(pLDDT_dict.values()), aa_seq, res_start])
    ...
    duplicated_residues = long_res_cnt + len(list(set(long_list)))*int(1400)
    fragment_count = long_cnt + len(list(set(long_list)))
...
# First, read the AFDB 
start_dir = os.getcwd()
in_prot = 'UP000005640_9606_HUMAN'  
dat = read_AFDB(in_prot)
...
if not os.path.exists(start_dir+'/out'):
    os.makedirs(start_dir+'/out')
os.chdir(start_dir+'/out')
output_dir = os.getcwd()

# Create the outupt file
fout = open((output_dir+'/'+'%s_pLDDT_scores.txt' % (in_prot)), 'w')
