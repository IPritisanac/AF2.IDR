...
sparta_seq('pred.tab', 'sparta_ACTR_AF')
...
sparta_to_ssp(cs_dict, 'sparta_ACTR_AF') 
...
#carbon_dat = np.genfromtxt('output_50253.txt') # phospho 4eb-2
carbon_dat = np.genfromtxt('output_5228.txt')
print(carbon_dat)   
...
def bmrb_to_ssp(bmrb_in, id):
    cs_order = ['CO', 'CA', 'CB']
    for i in range(np.shape(bmrb_in)[1] - 5):   # change to 4 for free ACTR
        cnt_bmrb = -1
...
exp = 5.71e-11 # m^2/s
exp_err = 0.02e-11 # m^2/s 
calc = 5.861e-11 # m^2/s
...
ax[0].plot(calc_intensity(5.71e-11)[0], calc_intensity(5.71e-11)[1], 'o-', mfc='white', label=r'measured')#, $D$ = 5.71 $\pm$ 0.02 $x$ 10$^{-11}$ m$^{2}$ $s^{-1}$ ')
ax[0].plot(calc_intensity(5.861e-11)[0], calc_intensity(5.861e-11)[1], '-', mfc='orange', mec='white', label='simulated')#, $D$ = 5.86 $x$ 10$^{-11}$ m$^{2}$ $s^{-1}$ ')
ax[0].legend(loc='upper right')
ax[0].set_xlabel(r'$G^{2}$/$G_{max}^{2}$', fontsize=12)
ax[0].set_ylabel(r'$I$/$I_{0}$', fontsize=12)

ax[1].errorbar(calc_intensity(5.71e-11)[1], calc_intensity(5.861e-11)[1], xerr=0.02/5.71*calc_intensity(5.71e-11)[1], fmt='o-')
ax[1].plot([np.min(calc_intensity(5.71e-11)[1]), np.max(calc_intensity(5.71e-11)[1])],[np.min(calc_intensity(5.71e-11)[1]), np.max(calc_intensity(5.71e-11)[1])], 'k-', label='y=x')
